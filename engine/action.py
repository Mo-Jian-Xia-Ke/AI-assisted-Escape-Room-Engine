...
class Action:
    def __init__(self, items, mode="hybrid", acceptance_threshold = 0.5):
    ...
    def interpret(self, new_input, labels=[]):
    ...
    def transformer_nlp_mode(self, labels):
        ...
        label, score = self.bart_interpret(labels)
    ...
    def hybrid_mode(self, labels):
        # print("Hybrid mode:")
        label = self.classic_nlp_mode(labels)
        if label == 'ask for hint' or label == 'other':
            label = self.transformer_nlp_mode(labels)
        return label
    ...
    def match_by_synonym(self, user_word):
        ...
        for item_name in self.items:
            if self.items[item_name].check_invisible():
                continue
